$('document').ready(function(){
    ...
    $('#submit_to_see_performance').on('click',function(e){
        ...
        if (total_question === '' || total_score === '') {
            console.log('Total question or total score is empty');
            return;
        }
        // total_question=0;
        // total_score=0;
        user=$('#username').val();
        if (total_question == null || total_question === undefined) {
            total_question = 0; // Set total_question to 0 if it's null or undefined
        }
        if (total_score == null || total_score === undefined) {
            total_score = 0; // Set total_score to 0 if it's null or undefined
        }
        e.preventDefault();
        $.ajax({
            type:'POST',
            ...
            $('#analyseModalLabel').html("Hello " + user + "<pre>" + data + "</pre>");
